...
**v3.9.3** (App) · Engine v1.8.0 · **Live:** [camaleon.bckthead3001.workers.dev](https://camaleon.bckthead3001.workers.dev) · [GitHub](https://github.com/dithmarpatpar300-lgtm/camaleon) · [ARCHITECTURE](ARCHITECTURE.md) · [SPEC](docs/SPEC.md) · [ROADMAP](docs/ROADMAP.md)
...
**Latest (v3.9.3):** **Wasm Sync Engine + S5 Redesign** — centralized detection of stale Wasm engines via `wasm-manifest.json` buildId comparison. Settings → Offline & cache completely redesigned with minimalist `SettingsRow` pattern, engine sync status badges, and auto-detect toggle. `build-wasm.mjs` now generates `wasm-manifest.json` with per-crate sizes and content hash buildId. Cache name centralized to single shared constant. See [docs/planning/wasm_sync_and_s5_redesign.md](docs/planning/wasm_sync_and_s5_redesign.md).
...
| **Wasm Sync Engine + S5 Redesign** | **v3.9.3** | ✅ Stale Wasm detection, manifest buildId, minimalist S5 |
