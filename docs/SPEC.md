...
**Status:** Active — Backend §5.8 refinements complete (v0.5.5); frontend UI/UX architecture & design system defined (§7, "Verde Camaleón")
...
### 7.4 UI/UX Architecture & Design System (Planned — UI track)

> **Status:** Design approved 2026-06-02 (identity, palette, routing, header scope). **Not yet implemented.** Delivery is phased UI-1..UI-5 (§7.8). This section is frontend doctrine; component code lands per phase.
...
### 7.5 Component Architecture (Planned)

Reusable, modular taxonomy under `frontend/src/`:

```
components/
├── ui/         # primitives: Button, IconButton, Badge, Card, Spinner,
│               #             Dropdown/Popover, SearchInput, Tooltip, Toast
├── layout/     # Header, ThemeToggle, LanguageSelector, Footer
└── transmute/  # ToolCard, ToolGrid, Dropzone, TransmutationPanel,
                # OptionsControls, Hero, PrivacyBanner
providers/      # ThemeProvider, I18nProvider
lib/tools/      # tool-registry.ts (single source of truth)
lib/i18n/       # EN/ES dictionaries
hooks/          # useTransmutationWorker (existing), useTheme, useI18n
```
...
### 7.7 Header Anatomy (Planned — first cut)

| Zone | Element |
|------|---------|
| Left | Chameleon logo mark + `Camaleon` wordmark |
| Center-left | `Transmutaciones` nav |
| Right | Language selector (EN / ES) |
| Right | Theme toggle (dark / light) |

**Deferred to later UI phases** (not in the first cut): live search / command palette, categorized "Transmutaciones" mega-menu, session counter, notification-sound toggle.

### 7.8 UI Implementation Track (Planned)

| Phase | Scope | Constraint |
|-------|-------|-----------|
| UI-1 | Design system foundation: tokens + `ThemeProvider` + `ui/` primitives + layout shell (Header + Footer) | No conversion behavior change; existing dropzone keeps working |
