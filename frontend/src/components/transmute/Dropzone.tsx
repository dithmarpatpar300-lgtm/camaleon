...
export function Dropzone({
  accept, status, dragging, sourceFileName,
  onDragOver, onDragLeave, onDrop, onFileSelect,
  idleLabel, processingLabel,
}: DropzoneProps) {
  const { t } = useI18n();
  ...
  return (
    <>
      ...
      <Card className="w-full">
        <CardBody>
          <div role="button" tabIndex={disabled ? -1 : 0} aria-label={t("dropzone.ariaLabel")} aria-disabled={disabled}
            ...
            className={`flex h-56 cursor-pointer flex-col items-center justify-center gap-3 rounded-xl border-2 border-dashed transition-colors ${
              disabled ? "border-border bg-bg-base cursor-not-allowed"
                : dragging ? "border-accent bg-accent-subtle" : "border-border bg-bg-base hover:border-text-muted"}`}>
            {disabled ? (
              <Spinner label={`${procText} ${sourceFileName ?? ""}`} />
            ) : (
              <>
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={1.5} className="h-8 w-8 text-text-muted" aria-hidden="true">
                  <path strokeLinecap="round" strokeLinejoin="round" d="M3 16.5v2.25A2.25 2.25 0 005.25 21h13.5A2.25 2.25 0 0021 18.75V16.5m-13.5-9L12 3m0 0l4.5 4.5M12 3v13.5" />
                </svg>
                <span className="text-sm text-text-muted">{dragging ? t("dropzone.dragLabel") : idleText}</span>
              </>
            )}
          </div>
        </CardBody>
