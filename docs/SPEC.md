...
**Status:** Active — MVP functional (JPEG↔PNG, app v0.6.4, engine v0.6.6). v1.0.0 gate: UI-5 (a11y/responsive). Optional engine: `refine_jpeg_encoder_swap` (§5.5.6)
...
#### 5.5.6 Encoder-swap doctrine (subsampling, Huffman, progressive)
...
§5.8 backend refinements through v0.6.6 are **complete**. Phase 4 (`v1.0.0`) per ROADMAP: UI-5 (a11y/responsive) + formal sign-off. Optional post-MVP engine: `refine_jpeg_encoder_swap` (§5.5.6) before chroma-subsampling UI.
...
> **Status:** Design approved 2026-06-02. **UI-1..UI-4 delivered (v0.6.1–v0.6.4).** UI-5 (accessibility + responsive sign-off) remains for v1.0.0 MVP gate per ROADMAP.
