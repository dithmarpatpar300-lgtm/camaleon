...
**Status:** v3.9.3 on `dev` (Wasm Sync Engine · S5 Offline Settings Redesign) · Engine v1.8.0 · **26 tools**
...
**S5 (v3.9.3): Wasm Sync Engine + Redesign** — complete UI redesign using `SettingsRow` pattern (consistency with S2–S8). New Wasm Sync Engine (`lib/wasm/wasm-sync.ts`) detects stale cached Wasm engines by comparing `wasm-manifest.json` `buildId` against stored `lastKnownWasmBuildId`. State machine: `idle → checking → up_to_date | stale → syncing → up_to_date | error`. Auto-detect toggle (`offline.wasmSyncEnabled`, default true) gates automatic manifest check on Settings drawer open. "Update engines" action purges `camaleon-wasm-v1` cache + re-runs `precacheFullToolkit`. "Check now" button for manual verification. `build-wasm.mjs` now generates `wasm-manifest.json` with content-hash `buildId` (SHA-256 of per-crate file sizes, 8 hex chars). Cache name centralized to `lib/wasm/wasm-cache-constants.ts` (single source of truth). New `OfflinePrefs` fields: `wasmSyncEnabled`, `lastKnownWasmBuildId`, `lastWasmSyncAt`. Reference: `docs/planning/wasm_sync_and_s5_redesign.md`.
...
**Wasm Sync Engine (v3.9.3):** The `camaleon-wasm-v1` cache uses `CacheFirst` — cached bytes are served indefinitely without revalidation. The Wasm Sync Engine solves stale Wasm by comparing a deployed `wasm-manifest.json` (generated by `build-wasm.mjs`) `buildId` against the stored `lastKnownWasmBuildId`. On mismatch, the user can purge + re-download all engines from Settings S5. Auto-detect is gated by `offline.wasmSyncEnabled` (default true). The engine does NOT poll — it checks on Settings drawer open or explicit "Check now" click. Cache name canonical source: `lib/wasm/wasm-cache-constants.ts`.
...
|---------|------|--------|---------|------------|
| 3.9.3-wasm-sync-s5 | 2026-06-26 | OpenCode | §7.13 S5 Wasm Sync Engine + complete redesign; §7.15 Wasm Sync Engine staleness detection via manifest buildId; `lib/wasm/wasm-sync.ts` state machine (idle/checking/up_to_date/stale/syncing/error); `build-wasm.mjs` generates `wasm-manifest.json` (SHA-256 content hash buildId); cache name centralized to `lib/wasm/wasm-cache-constants.ts`; new OfflinePrefs: `wasmSyncEnabled`, `lastKnownWasmBuildId`, `lastWasmSyncAt`; 191 Vitest tests. | `docs/planning/wasm_sync_and_s5_redesign.md` |
